SELECT 
  ...
  (MAX(main_order_day)::date)- (MIN(main_order_day)::date) as cust_order_diff,
--  AVG(real_item_amount) as avg_monetary_value,
  SUM(real_item_amount) as sum_monetary_value,
--  (('2021-07-29'::date) - MIN(main_order_day)::date) as first_buy_interval,
  (('2021-07-29'::date) - MAX(main_order_day)::date) as last_buy_interval
into temp table RFM_data
FROM public.ec2_order_item_detail_all
where 
    Sales_Status = '銷貨' and 
    Profit_Center = 'C001' and 
    SAP_Product_Line Not In ('99_其他')
GROUP BY member_id;
...
into temp table trpt_member_rfm2
from RFM_data;


select * from trpt_member_rfm2;
